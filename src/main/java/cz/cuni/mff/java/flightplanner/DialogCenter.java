...
public class DialogCenter {
    ...
    static @NotNull List<Plugin> choosePlugins(@NotNull List<Plugin> modes){
        ...
        var keyWordOpt = modes  .stream()
                                    .max(Comparator.comparingInt(plugin -> plugin.keyword().length()));
        int longestKeyword;
        if (keyWordOpt.isPresent()) {
            longestKeyword = keyWordOpt.get()
                                       .keyword()
                                       .length();
        } else {
        ...
        options = Arrays.stream (line.split(",")) // in the stream, the array is corrected for the blank fields
                        ...
                        .map    (String::strip)
                        .toArray(String[]::new);
        List<Plugin> matchingList = new LinkedList<>();
            ...
            try {
                ...
                switch (matchingList.size()) {
                    ...
                    case 1:
                        List<Plugin> toAdd =
                                matchingList.stream()
                                            .filter(x -> !activePlugins.contains(x))
                                            .collect(Collectors.toList());
                        activePlugins.addAll(toAdd);
                        modes.removeAll(toAdd);
                        break;
                    default:
                        System.out.println("Multiple possible options were found. Please, choose only among these now.");
                        activePlugins.addAll(choosePlugins(matchingList)
                                                .stream()
                                                .filter(x -> !activePlugins.contains(x))
                                                .collect(Collectors.toList())
                                            );
     ...
     *                   evalute as true.
     *
     * @param blankAllowed  Flag to indicate whether the empty line will be
     *                      accepted as a correct input.
     *
